...
export async function searchPatientsByName(name) {
  const response = await fetch(`${SEARCH_SERVICE_URL}/api/search/patients?name=${encodeURIComponent(name)}`);
...
export async function searchPatientsByCondition(condition) {
  const response = await fetch(`${SEARCH_SERVICE_URL}/api/search/patients/by-condition?condition=${encodeURIComponent(condition)}`);
...
export async function advancedSearch(params) {
  ...
  const response = await fetch(`${SEARCH_SERVICE_URL}/api/search/patients/advanced?${query.toString()}`);
  if (!response.ok) throw new Error("Failed advanced search");
  return response.json();
}

export async function getDoctorOverviewByDay(doctorUserId, date) {
  const response = await fetch(`${SEARCH_SERVICE_URL}/api/search/doctor/${doctorUserId}/overview-by-day?date=${encodeURIComponent(date)}`);
